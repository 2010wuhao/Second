 ...
 * ----------      ------------     -------------     -----------
 * benz          2014-1-16           1.0         Check for NULL, 0 h/w
 * =====================================================================
 **/
...
public class JStyleDialog extends Dialog {

	private static final String TAG = JLog.makeTag(JStyleDialog.class);
	...
	public JStyleDialog(Context context) {
		super(context);
	}

	public JStyleDialog(Context context, int theme) {
		super(context, theme);
	}

	public JStyleDialog(Context context, boolean cancelable, OnCancelListener cancelListener) {
		super(context, cancelable, cancelListener);
	}

	public JStyleDialog(Context context, boolean cancelable, Message cancelCallback) {
	...
	public boolean prepareBlur() {
		...
		try {
			if (mViewToBlur != null) {
				...
				JLog.d(TAG, "onStart blur image width = " + localBitmap2.getWidth()+ " height = " + localBitmap2.getHeight());
			...
			ret = false;
			JLog.d(TAG, "prepareBlur error="+e.toString());
		}
		return ret;
	}
	...
	protected void onStart() {
		...
		try {
			...
			decorView.addView(mViewBeBlur, 0, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.MATCH_PARENT));
		} catch (Exception e) {
			JLog.e(TAG, "createBlur", e);
		}
	}
	...
	private AnimationSet getInAnimation() {
		if (mInAnimation == null) {
			...
			new ScaleAnimation(1.2f, 1.0f, 1.2f, 1.0f,
