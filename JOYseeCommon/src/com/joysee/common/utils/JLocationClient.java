...
public class JLocationClient {
    ...
    private Proxy getProxy() {
        if (proxy == null) {
            ...
            String host = android.net.Proxy.getDefaultHost();
            int port = android.net.Proxy.getDefaultPort();
            JLog.d(TAG, " host : " + host + "     |||   port " + port);
        ...
        try {
            if (!mIsInitialized) {
                ...
                if (isRunning()) {
                    ...
                    if (isRunning()) {
                        JLog.d(TAG, ">>>>>>>>>>JSON : " + conn.getResponseCode());
                        JLog.d(TAG, ">>>>>>>>>>JSON : " + str);
                        //JLog.d(TAG, ">>>>>>>>>>JSON : " + JUnicode.decode(str));
                    }
                ...
                JLog.d(TAG, "JLocation has already initialized..");
                if (mCallback != null) {
                    new Handler(mContext.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
            ...
            JLog.d(TAG, "http request failed! " + e);
            if (mCallback != null) {
                new Handler(mContext.getMainLooper()).post(new Runnable() {
                    @Override
                    public void run() {
    ...
    boolean parseLocation(String s) throws JSONException {
        ...
        JLog.d(TAG, "city = " + mCity);
        JLog.d(TAG, "city_code = " + mCityCode);
        JLog.d(TAG, "province = " + mProvince);
        ret = true;
        return ret;
    }
}
