...
public class PortalCursor extends ImageView implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    private class CursorTarget {
        PortalCursor.AlignType mAlignType;
        boolean mIgnoreTgtSize;
        View mTgtView;

        public CursorTarget(View target, boolean ignore, PortalCursor.AlignType align) {
            this.mTgtView = target;
            this.mIgnoreTgtSize = ignore;
    ...
    private class CusorStopNotifier implements Runnable {
        ...
        public void run() {
            PortalCursor.this.doNotifyStop();
        }
    }
    ...
    private static final String TAG = JLog.makeTag(PortalCursor.class);
    ...
    public PortalCursor(Context context, AttributeSet attr) {
        super(context, attr);
        setScaleType(ImageView.ScaleType.FIT_XY);
        setBackgroundResource(R.drawable.portal_title_focus_bg);
    ...
    private void mov2Target(CursorTarget target, boolean animator) {
        ...
        JLog.d(TAG, "mov2Target (" + this.mTargetPos[0] + ", " + this.mTargetPos[1] + ") from ("
        ...
        if (animator) {
            ...
            JLog.d(TAG, "selfCenter (" + selfCenter[0] + ", " + selfCenter[1] + ") targetCenter ("
        ...
        if (!target.mIgnoreTgtSize) {
            ...
            if ((targetWidthRadioed == bgBoundsRect.width())
                ...
                JLog.d(TAG, "set2Target ignore the same size scale");
            } else {
    ...
    public void move2Target(View dest, boolean ignore, AlignType align) {
        ...
        if (this.mTgtQue.size() > 0) {
            JLog.d(TAG, "Triger animation now");
            trigerMov();

            JLog.d(TAG, "Animation on going just save to target Queue size:" + this.mTgtQue.size());
        }
    }
