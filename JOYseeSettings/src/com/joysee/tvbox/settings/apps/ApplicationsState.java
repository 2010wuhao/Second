 ...
 * ----------      ------------     -------------     -----------
 * wumingjun         @Jun 10, 2014            1.0      Check for NULL, 0 h/w
 * =====================================================================
 **/
...
public class ApplicationsState {
    ...
    final static CanBeOnSdCardChecker mCanBeOnSdCardChecker = new CanBeOnSdCardChecker();
        ...
        boolean ensureIconLocked(Context context, PackageManager pm) {
            ...
            if (this.icon == null) {
                if (this.apkFile.exists()) {
                    ...
                    /*
                     * try { this.time = pm.getPackageInfo(info.packageName,
                    ...
                    this.mounted = false;
                    this.icon = context.getResources().getDrawable(com.android.internal.R.drawable.sym_app_on_sd_unavailable_icon);
                }
    ...
    public void resume(Callbacks callbacks) {
        if (DEBUG_LOCKING)
        ...
        synchronized (mEntriesMap) {
            ...
            // Do not display some APKs with Phone, when there is no phone( dgt
            // edited)
            if (!isVoiceCapable(mContext)) {
                ...
                for (ApplicationInfo appInfo : mApplications) {
                    if (appInfo.packageName.equalsIgnoreCase("com.android.phone") || appInfo.packageName.equalsIgnoreCase("com.android.providers.telephony")
                            || appInfo.packageName.equalsIgnoreCase("com.android.contacts") || appInfo.packageName.equalsIgnoreCase("com.android.providers.contacts")) {
                        delList.add(appInfo);
                    }
                }
            ...
            // if(isSanAo){
            // List<ApplicationInfo> sanaoList = new
    ...
    private long getTotalExternalSize(PackageStats ps) {
        if (ps != null) {
            return ps.externalCodeSize + ps.externalDataSize + ps.externalMediaSize + ps.externalObbSize + ps.externalCacheSize;
        }
        return SIZE_INVALID;
    }
                ...
                synchronized (mEntriesMap) {
                    ...
                    if (entry != null) {
                        synchronized (entry) {
                            ...
                            long externalDataSize = stats.externalDataSize + stats.externalMediaSize + stats.externalCacheSize;
                            long newSize = getTotalExternalSize(stats) + getTotalInternalSize(stats);
                            if (entry.size != newSize || entry.cacheSize != stats.cacheSize || entry.codeSize != stats.codeSize || entry.dataSize != stats.dataSize
                                    || entry.externalCodeSize != externalCodeSize || entry.externalDataSize != externalDataSize) {
                                entry.size = newSize;
                                ...
                                if (DEBUG)
                                    Log.i(TAG, "Set size of " + entry.label + " " + entry + ": " + entry.sizeStr);
                                MyLog(" onGetStatsCompleted = " + entry.label + "mTotalSize = " + entry.size + " mAppEntry.codeSize = " + entry.codeSize + " mDataSize = " + entry.dataSize
                                        + " mCacheSize = " + entry.cacheSize + " externalSize = " + entry.externalSize + " internalSize = " + entry.internalSize + " externalDataSize = "
                                        + externalDataSize + " externalCodeSize = " + externalCodeSize);
                                sizeChanged = true;
                            }
                        }
                        if (sizeChanged) {
    ...
    public static long getAppLastDateFromProvider(Context context, String packageName) {
        long date = 0;
        Cursor cursor = context.getContentResolver().query(CONTENT_URI, null, COLUMN_NAME_PACKAGE_NAME + " = ?", new String[] { packageName }, null);
        if (cursor != null) {
            if (cursor.getCount() != 0) {
                cursor.moveToFirst();
                date = cursor.getLong(cursor.getColumnIndex(COLUMN_NAME_LAST_USED_DATE));
            }
            cursor.close();
        }
        return date;
    ...
    final static class CanBeOnSdCardChecker {
        ...
        boolean check(ApplicationInfo info) {
            ...
            if ((info.flags & ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0) {
                ...
                if ((info.flags & ApplicationInfo.FLAG_FORWARD_LOCK) == 0 && (info.flags & ApplicationInfo.FLAG_SYSTEM) == 0) {
                    if (info.installLocation == PackageInfo.INSTALL_LOCATION_PREFER_EXTERNAL || info.installLocation == PackageInfo.INSTALL_LOCATION_AUTO) {
                        canBe = true;
